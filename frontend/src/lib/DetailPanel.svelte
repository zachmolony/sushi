<script lang="ts">
  ...
  $: existingNotApplied = $tagsWithCounts
    .filter((t) => !assetTagNames.includes(t.name))
    .map((t) => t.name)
    .slice(0, 6);
  $: prebuiltNotApplied = SUGGESTED_TAGS.filter(
    (t) => !assetTagNames.includes(t) && !existingNotApplied.includes(t),
  );
  $: tagSuggestions = [
    ...existingNotApplied,
    ...prebuiltNotApplied.slice(0, Math.max(0, 8 - existingNotApplied.length)),
  ];
...
{#if $selectedAsset && $detailPanelOpen}
  <aside
    class="relative w-[300px] min-w-[300px] bg-base-800 p-5 flex flex-col gap-4 border-l border-white/[0.06] overflow-y-auto"
  >
    <!-- Close button -->
    <button
      class="absolute top-3 right-3 bg-transparent border-none text-white/30 hover:text-white/70 cursor-pointer text-base p-0 leading-none z-10 transition-colors"
      on:click={closeDetailPanel}
      title="Close panel">✕</button
    >
    <!-- Preview -->
    <div
      class="w-full aspect-square bg-black/30 rounded-lg overflow-hidden flex items-center justify-center"
    >
    ...
    <div class="flex items-start gap-2">
      ...
      <button
        class="bg-transparent border-none text-xl cursor-pointer p-0 leading-none shrink-0 transition-all hover:scale-110
          ...
          ? 'text-fav'
          : 'text-white/30 hover:text-yellow-400/80'}"
        on:click={() => toggleFavorite($selectedAsset.id)}
        title={$selectedAsset.favorited === 1
    ...
    <div class="flex flex-col gap-1.5">
      <div class="flex flex-col gap-0.5">
        <span class="text-[0.65rem] uppercase tracking-wider opacity-40"
          >Path</span
        >
        <code class="text-[0.78rem] break-all leading-relaxed opacity-80"
          >{$selectedAsset.absolute_path}</code
        >
      </div>
      <div class="flex flex-col gap-0.5">
        <span class="text-[0.65rem] uppercase tracking-wider opacity-40"
          >Size</span
        >
        <span class="text-[0.78rem] opacity-80"
          >{formatSize($selectedAsset.file_size)}</span
        >
      </div>
      {#if $selectedAsset.poly_count > 0}
        <div class="flex flex-col gap-0.5">
          ...
          >
          <span class="text-[0.78rem] opacity-80"
            >{formatPoly($selectedAsset.poly_count)} tris</span
          >
        </div>
      {/if}
      <div class="flex flex-col gap-0.5">
        ...
        >
        <span class="text-[0.78rem] opacity-80"
          >{new Date($selectedAsset.modified_at).toLocaleDateString()}</span
        >
      </div>
    </div>
    ...
    <div class="flex flex-col gap-1.5">
      ...
      <div class="flex flex-wrap gap-1 items-center">
        {#each $selectedAssetTags as tag}
          <span
            class="inline-flex items-center gap-1 px-2 py-0.5 bg-surface border border-surface-border rounded-full text-[0.7rem] text-white"
          >
            {tag.name}
            <button
              class="bg-transparent border-none text-white/40 hover:text-red-400 cursor-pointer text-[0.6rem] p-0 leading-none"
              on:click={() => removeTag(tag.id)}>✕</button
            >
          </span>
        {/each}
        ...
        <div class="flex flex-wrap gap-1">
          {#each filteredTagSuggestions as tag}
            <button
              ...
              on:click={() => applyTagSuggestion(tag)}>{tag}</button
            >
          {/each}
        </div>
      {/if}
      ...
      <div class="flex flex-wrap gap-1 items-center">
        {#each $selectedAssetCollections as col}
          ...
          >
            ...
            <button
              class="bg-transparent border-none text-white/40 hover:text-red-400 cursor-pointer text-[0.6rem] p-0 leading-none"
              on:click={() => removeSelectedFromCollection(col.id)}>✕</button
            >
          </span>
        {/each}
    ...
    <div class="flex flex-col gap-1.5 mt-auto">
      <button
        ...
        title={$blenderConnected
          ? "Import into Blender"
          : "Blender addon not running"}
      ...
      {#if !confirmDelete}
        <button
          class="px-3 py-2 rounded-md text-sm bg-surface border border-red-900/40 text-red-400/70 text-left cursor-pointer font-inherit hover:bg-red-950/40 hover:text-red-400 transition-colors"
          on:click={() => (confirmDelete = true)}>🗑 Delete Asset</button
        >
      {:else}
        <div class="flex gap-1.5">
          <button
            class="flex-1 px-3 py-2 rounded-md text-sm bg-red-900/40 border border-red-700/50 text-red-300 text-center cursor-pointer font-inherit hover:bg-red-800/50 transition-colors"
            on:click={() => {
              confirmDelete = false;
              deleteSelectedAsset();
          ...
          >
          <button
            class="px-3 py-2 rounded-md text-sm bg-surface border border-surface-border text-white/60 text-center cursor-pointer font-inherit hover:bg-surface-hover transition-colors"
            on:click={() => (confirmDelete = false)}>Cancel</button
          >
        </div>
      {/if}
    </div>
